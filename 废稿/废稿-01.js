const tmp = { 
    ...
    isLook_f:true,
		...
		shengID:'#sheng',
    xueliID:'#fxk-111',
    ruxuenianfenID:'#fxk-222',
    zaizhiId:'#fxk-333',
    nianfenID:'#fxk-444',
		shiID:'#shi',
		dataChild:[],
    education:['高中', '专科', '本科', '研究生','博士或以上'],
    enrollmentDate:['70后','80后','85后','90后','95后','00后'],   
    job:['技术', '产品', '设计', '测试', '运维', '其他'],
    ...
    $('body').on('click','.accomplish-bth',this.syxmwc);	
		$('.showMask').on('click',this.showMask);//点击注册
		$('body').on('click','.fill-gender-item',this.gender);//选择性别
		$('body').on('click','#sheng',this.sheng);//省
		$('body').on('click','#shi',this.shi);//市
    $('body').on('click','#xueSheng',this.add);//身份点击按钮
    $('body').on('click','#zaiZhi',this.bdd);//职业点击按钮
    $('body').on('click','#daiYe',this.cdd);//待业点击按钮
    $('body').on('click','#fxk-111',this.xueli);//学历选择按钮
    $('body').on('click','#fxk-222',this.ruxuenianfen);//入学年份选择按钮
    $('body').on('click','#fxk-333',this.zhiye);//职业选择按钮
    $('body').on('click','#fxk-444',this.nianfen);//入职年份选择按钮
   
    $('body').on('click','#xiayibu_1',this.xiayibu_1);//页面1的下一步按钮
    $('body').on('click','#shangyibu_1',this.shangyibu_1);//页面2的上一步按钮
    $('body').on('click','#shangyibu_2',this.shangyibu_2);//页面2的下一步按钮
    $('body').on('click','#C_shangyibu',this.C_shangyibu);//页面3的上一步按钮
    $('body').on('click','#C_xiayibu',this.C_xiayibu);//页面3的申请认证按钮
    $('body').on('click','.binding-button',this.bindingButton);//最后一步绑定按钮


    $('body').on('click','.select-item',this.selectItem);//第二步 职业选择方向按钮self-assessment-items
    $('body').on('click','.self-assessment-items',this.selfAssessmentItems);//第三步 水平选择按钮
  ...
  showMask:function(){//点击注册
  	...
  	$('body').append(html);
  	// tmp.showFirstTemplete();
   //  tmp.heightLightBtn();
  },
  createMaskHTML:function(){//点击注册插入html
  	let gender = tmp.data.gdender;
  	let html = `
  	<div class="registration-page_container" style="">
    <div class="banner-container">
          ...
          <div class="register-content_one" style="">
            ...
            <div class="portrait-fill">
              <div class="fill-align_top">
                <div class="fill-gender">
                  <span>性别：</span>                         
                  <div id="gender" class="fill-gender-item">女</div>            
                </div>
                <div class="fill-gender">
                  <span>居住地：</span>           
                  <div id="sheng" class="fill-gender-item">省</div>                      
                  <div id="shi" class="fill-gender-item">市</div>            
                </div>
              </div>
              <div class="fill-align_middle">
                <p class="state-items">您的当前状态 :</p>

                 <div class="state-list">               
                  <div class="state-item" id="xueSheng">学生</div>
                  <div class="fill-detail-list"  style="" id="fxk-1">
                    <div id="fxk-111" class="fill-detail-item">你所在的年级</div>
                    <div id="fxk-222" class="fill-detail-item">入学年份</div>  
                  </div>
                </div>

                <div class="state-list">
                  <div class="state-item" id="zaiZhi">在职</div>
                  <div class="fill-detail-list" style="" id="fxk-2">
                    <div id="fxk-333" class="fill-detail-item">您所从事的职业</div>
                    <div id="fxk-444" class="fill-detail-item">年份</div>  
                  </div>
                </div>
                <div class="state-list">
                  <div class="state-item" id="daiYe">待业</div>
                </div>
              </div>
            </div>
            <div class="fill-align_buttom">
              <a id="xiayibu_1" class="next-step_button" href="javascript:;">下一步</a>
            </div>        
          </div>

          <div class="register-content_two" style="display: none;">
            <p class="register-select-hint">选择你感兴趣的职业方向（最多可选择 3 项）
              <span>系统将根据您的选择为您推荐合适的课程</span>
            </p>
            <ul class="select-list">
              <li class="select-item">Web 前端工程师<i></i></li>
              <li class="select-item">Android 开发工程师<i></i></li>
              <li class="select-item">iOS 开发工程师<i></i></li>
              <li class="select-item">GO 语言工程师<i></i></li>
              <li class="select-item">Python Web工程师<i></i></li>
              <li class="select-item">PHP 工程师<i></i></li>
              <li class="select-item">Java Web工程师<i></i></li>
            </ul>
            <div class="select-align_buttom">
              <a id="shangyibu_1" class="select-step_button" href="javascript:;">上一步</a>
              <a id="shangyibu_2" class="select-step_button" href="javascript:;">下一步</a>
            </div>
          </div>

          <div class="register-content_three" style="display: none;">
            <p class="self-assessment-hint">您在这些方向上当前的水平如何？
              <span>系统将根据您的选择为您推荐合适的课程</span>
            </p>
            <div class="self-assessment-list">
              <ul class="self-assessment-item">
                <li class="self-items-name">Web 前端工程师<i></i></li>
                <li class="self-assessment-items">完全不了解<i></i></li>
                <li class="self-assessment-items">有点了解<i></i></li>
                <li class="self-assessment-items">熟悉<i></i></li>
                <li class="self-assessment-items">精通<i></i></li>
              </ul>
              <ul class="self-assessment-item">
                <li class="self-items-name">iOS 前端工程师<i></i></li>
                <li class="self-assessment-items">完全不了解<i></i></li>
                <li class="self-assessment-items">有点了解<i></i></li>
                <li class="self-assessment-items">熟悉<i></i></li>
                <li class="self-assessment-items">精通<i></i></li>
              ...
              <ul class="self-assessment-item">
                <li class="self-items-name">PHP 前端工程师<i></i></li>
                <li class="self-assessment-items">完全不了解<i></i></li>
                <li class="self-assessment-items">有点了解<i></i></li>
                <li class="self-assessment-items">熟悉<i></i></li>
                <li class="self-assessment-items">精通<i></i></li>
            ...
            <div class="self-assessment-button">
              <a id="C_shangyibu" class="self-assessment-bth" href="javascript:;">上一步</a>
              <a id="C_xiayibu" class="self-assessment-bth" href="javascript:;">申请认证</a>
            </div>
          </div>
      </div>
      <div class="binding-section" style="display: none;">
        <div class="register-close-button"><i></i></div>  
      ...
      <div class="accomplish-section" style="display: none;">
        <div class="register-close-button"><i></i></div>  
  ...
  gender:function(e){//选择性别
  	let gender = tmp.data.gender;
  	let ID = tmp.data.genderID;
  	if(tmp.data.isLook){
  		let html;
    for (let i = 0; i< gender.length; i++) {
        html += `<option class="gender-desc">  ${gender[i]}  </option>`;
    }
    let arr = document.createElement("select");
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
    tmp.data.isLook = false;
  	}
  },
  sheng:function(){//省
  	let dataChild = tmp.data.data_area.child;
  	let ID = tmp.data.shengID;
  	if(tmp.data.isLook_a){
  		let html;
    for (let i = 0; i< dataChild.length; i++) {
        html += `<option>  ${dataChild[i].name}  </option>`;  
        if(dataChild[i].child){
         dz = dataChild[i].child;
        }
    }   
    $('#shi').add(dz.name)
    tmp.data.shi = dz.name;
    let arr = document.createElement("select");
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
    tmp.data.isLook_a = false; 
  	}
  },
  shi:function(){//市
  	let child = dz;  	
  	let ID = tmp.data.shiID;
  	if(tmp.data.isLook_b){
  		let html;
    for (let i = 0; i<child.length; i++) {
    	 html += `<option>  ${child[i].name}  </option>`;  
    }
    let arr = document.createElement("select");
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
    tmp.data.isLook_b = false;
  	}
  },
  xueli:function(){//学历选择按钮
   if(tmp.data.isLook_c){
    let education = tmp.data.education; 
    let ID = tmp.data.xueliID
    let html;
    for(let i = 0; i < education.length; i++)  {
      // let item = `<li class="fill-desc">${education[i]}</li>`
      html+= `<option>  ${education[i]}  </option>`;
    }
    let arr = document.createElement("select");
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
     tmp.data.isLook_c = false;
   }
  },
  ruxuenianfen:function(){//入学年份选择按钮
    if(tmp.data.isLook_d){
     let enrollmentDate = tmp.data.enrollmentDate; 
    let ID = tmp.data.ruxuenianfenID;
    let html;
    for(let i = 0; i < enrollmentDate.length; i++)  {
      // let item = `<li class="fill-desc">${enrollmentDate[i]}</li>`
      html+= `<option>  ${enrollmentDate[i]}  </option>`;
    }
    let arr = document.createElement("select");
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
     tmp.data.isLook_d = false;
   }
  },
  zhiye:function(){//职业选择按钮
   if(tmp.data.isLook_e){
     let job = tmp.data.job; 
    let ID = tmp.data.zaizhiId;
    let html;
    for(let i = 0; i < job.length; i++)  {
      // let item = `<li class="fill-desc">${job[i]}</li>`
      html+= `<option>  ${job[i]}  </option>`;
    }
    let arr = document.createElement("select");
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
     tmp.data.isLook_e = false;
   }
  },
  nianfen:function(){//入职年份选择按钮
    if(tmp.data.isLook_f){
    let enrollmentDate = tmp.data.enrollmentDate; 
    let ID = tmp.data.nianfenID;
    let html;
    for(let i = 0; i < enrollmentDate.length; i++)  {
      // let item = `<li class="fill-desc">${enrollmentDate[i]}</li>`
      html+= `<option>  ${enrollmentDate[i]}  </option>`;
    ...
    arr.innerHTML = html;    
    document.querySelector(ID).appendChild(arr);
     tmp.data.isLook_f = false;
   }
  },
  xiayibu_1:function(){//页面1的下一步按钮  
  ...
  shangyibu_1:function(){//页面2的上一步按钮
  ...
  shangyibu_2:function(){//页面2的下一步按钮
    $('.register-content_two').css("display","none"); 
    $('.register-content_three').css("display"," inline-block");
    $('#procedure-02').css("display","none");
    $('#procedure-03').css("display","inline-block");
  },
  C_shangyibu:function(){//页面3的上一步按钮
    $('.register-content_three').css("display","none");
    $('.register-content_two').css("display"," inline-block");
    $('#procedure-03').css("display","none");
    $('#procedure-02').css("display","inline-block");  
  },
  C_xiayibu:function(){//页面3的申请认证按钮
    $('.register-section').css("display","none");
    $('.binding-section').css("display"," inline-block");
  },
  bindingButton:function(){//最后一步绑定按钮
    $('.binding-section').css("display","none");
    $('.accomplish-section').css("display"," inline-block");
  },
  selectItem:function(){//第二步 职业选择方向按钮
    if($(this).hasClass('select-item-active')) {
      let careerDirection = tmp.data.careerDirection;
      for(let i=0;i<careerDirection.length;i++){
        if(careerDirection[i] == this){
      ...
      careerDirection.push(this);
      if(careerDirection.length>3){
        $(careerDirection[0]).removeClass('select-item-active');
        careerDirection.shift()
      }
    }
  }, 
  selfAssessmentItems:function(){    
    ...
    for (let i = listItem.length - 1; i >= 0; i--) {
      $(listItem[i]).removeClass('self-item-active')  
    }
    // for (let i = itemLine.length - 1; i >= 0; i--) {
    //   $(itemLine[i]).removeClass('item-line-active')  
    // }
    $(this).addClass('self-item-active');
    for (let i = listItem.length - 1; i >= 0; i--) {
    ...
    let directionLevel = tmp.data.directionLevel;   
    for (let i = directionLevel.length - 1; i >= 0; i--) {
